...
class _AddEventState extends State<AddEvent> {
  ...
  bool isEnrollAvailable =
      false; // Default value, can be changed via UI if needed
          ...
          child: SingleChildScrollView(
            child: Column(
              children: [
                ...
                const SizedBox(height: 16),
                ListTile(
                  title: Text(selectedPhoto != null
                      ? 'URL zdjęcia: ${selectedPhoto!.path}'
                      : 'URL zdjęcia'),
  ...
  void _addEvent() async {
    if (selectedDate == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
            content: Text('Proszę wybrać datę i godzinę wydarzenia.')),
      );
      return;
    }
    ...
    if (selectedPhoto != null) {
      ...
      String folderPath =
          'event_photos/${eventRef.id}'; // Use the event ID as the folder name
